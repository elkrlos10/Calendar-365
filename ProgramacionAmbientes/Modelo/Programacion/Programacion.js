    ...
    ['$scope', '$rootScope', '$location', '$cookies', '$cookieStore', 'ProgramacionService', '$routeParams', '$sce', '$http', '$notify',
        function ($scope, $rootScope, $location, $cookies, $cookieStore, ProgramacionService, $routeParams, $sce, $http, $notify) {
            ...
            if ($rootScope.globals.currentUser.tipousuario == 1) {
                ////administrador
                $(".administrador").css("display", "grid");
                $(".coordinador").css("display", "none");
            ...
            $('#HoraInicioA').datetimepicker({
                format: 'HH:mm'
            });

            $('#HoraFinA').datetimepicker({
                format: 'HH:mm'
            });
            //------------------------------------------------------------------------------------------------------------
                        ...
                        if (response.success == true) {
                            ...
                            $.each(response.programacion, function (index, value) {
                                ...
                                $scope.events.push({
                                    ...
                                    allDay: false,
                                    color: value.Color,
                                    tip: "PROGRAMA: " + value.Programa.toUpperCase() + " - " + "INSTRUCTOR:" + value.NombreInstructor.toUpperCase() + " - " + "COMPETENCIA:" + value.Competencia.toUpperCase()
                                });
            ...
            $scope.Disponibilidad1 = function () {
                ...
                if ($scope.Programacion1.Jornada == "" || $scope.Programacion1.HoraInicio == "" || $scope.Programacion1.HoraFin == "") {
                    ...
                    ProgramacionService.Disponibilidad1($scope.Transversal.Transversal, $rootScope.globals.currentUser.idpersona, $scope.Programacion1, $scope.Programacion.IdAmbiente, function (response) {
                        if (response.success == true) {
                        ...
                        $.each($scope.Ambiente, function (index, value) {
                            $.each($scope.Sede, function (index1, value1) {
                                if (value1.TipoSede == 2 && value1.IdSede == $scope.Sede.IdSede) {
                                    ...
                                    $scope.NombreCampo = "Nombre Empresa"
                                    ingreso++;
                                } else {
            ...
            $scope.ConsultarAmbientesCoordinador = function (IdCoordinador) {
                ...
                ProgramacionService.ConsultarAmbientes(1, function (response) {
                    if (response.success == true) {
            ...
            $scope.FiltroCoordinador = function (Sede1, ambiente) {
                if (Sede1.IdSede == undefined || $scope.RangoHoras.Jornada == null || ambiente.IdAmbiente == undefined) {
                    bootbox.dialog({
                        title: "Información",
                    ...
                    ProgramacionService.FiltroProgramacionxCoordinador($scope.RangoHoras.Jornada, ambiente.IdAmbiente, function (response) {
                        if (response.success == true) {
            ...
            //#region Envio de correo Instructor
            ...
            $scope.PrestamoLlaves = function () {
                ...
                $scope.progrmacionSelec.CedulaIns = $scope.cedula;
                if ($("#Observacion").is(":visible")) {
                    $scope.progrmacionSelec.opc = 1;
                }
            ...
            $scope.AbrirModalEliminarPrestamo = function () {
                ...
                if (ProgramacionBorrar.length != 1) {
                    ...
                    $scope.Programacioneliminar = ProgramacionBorrar;
                }
            }

            $scope.EliminarPrestamol = function () {
                ProgramacionService.EliminarReciboLlaves($scope.Programacioneliminar[0], function (response) {
                    if (response) {
                        $scope.datalists = response.datos;
                        $scope.ListaCompleta = response.datos;
                        $.each($scope.datalists, function (index, value) {
            ...
            $scope.ModalPrestamoAmbiente = function (posicion, opc) {
                ...
                if (opc == 1) {
                    ...
                    $scope.Ob.mensaje = "¿Desea confirmar la recepción de las llaves?";
                    $scope.AmbienteSelec.Entrego = true;
                    $("#Observacion1").hide();
                }
            }

            $scope.PrestamoLlavesAmbientes = function () {
                $scope.AmbienteSelec.Observacion = $scope.Ob.Observacion;

                ProgramacionService.GuardarPrestamoLLavesAmbientes($scope.AmbienteSelec, function (response) {
                    if (response.success) {
                        $scope.datalists = response.datos;
                        $.each($scope.datalists, function (index, value) {
            ...
            $scope.CancelarLlavesAmbiente = function () {
                if ($scope.AmbienteSelec.Entrego == true) {
                    $scope.AmbienteSelec.Entrego = false;
                } else {
                    $scope.AmbienteSelec.Recibio = false;
                }
            }
                ...
                ProgramacionService.AmbientesEntregarLlaves(function (response) {
                    if (response) {
                        ...
                        $.each($scope.datalists, function (index, value) {
                            ...
                            if ($scope.datalists[index].Recibio == true) {
                                $("input[name='recibio" + index + "']").prop("disabled", true);
                            }
                            //Variable para setear la paginación 
                            $scope.curPage = 0;
                            ...
                            if (response.datos.length>0) {
                                $.each(response.datos, function (index, value) {
                                    ...
                                    $scope.AmbientesExport.push({
                                        Sede: value.NombreSede, Area: value.Area,
                                        Piso: parseInt(value.Piso),
                                        Ambiente: value.Ambiente, Numero_Equipos: value.NumeroEquipos,
                                        Numero_Mesas: value.NumeroMesas, Numero_Sillas: value.NumeroSillas
                                    });
                                });

                                alasql('SELECT * INTO XLSX("Ambientes Sin Programacion.xlsx",{headers:true}) FROM ?', [$scope.AmbientesExport]);
                            } else {
                                bootbox.dialog({
                                    title: "Información",
                                    message: "En el horario ingresado no hay ambientes disponibles.",
                                    buttons: {
                                        success: {
                                            label: "Cerrar",
                                            className: "btn-primary",
